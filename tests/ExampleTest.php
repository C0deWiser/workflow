...
class ExampleTest extends TestCase
{
    ...
    public function testBasics()
    {
        ...
        $this->assertEquals('state', $post->workflow()->getAttributeName());
        $this->assertNull($post->workflow()->getState(), 'Initial state is not initialized');

        $this->assertEquals('next', $post->workflow('next')->getAttributeName(), 'Resolving correct blueprint');

        // Implicit init (using observer)
        $this->assertTrue((new StateMachineObserver)->creating($post));
        $this->assertEquals($post->workflow()->getInitialState(), $post->state, 'State value was initialized on creating event');
    }
    ...
    public function testTransitions()
    {
        ...
        $this->assertCount(6, $post->workflow()->getTransitions());
    }
    ...
    public function testRelevantTransitions()
    {
        ...
        $this->assertCount(2, $post->workflow()->getRelevantTransitions());

        $post->workflow()->getRelevantTransitions()
            ->each(function (Transition $transition) use ($post) {
                // Assert that every relevant transition starts from current state
                $this->assertEquals($post->state, $transition->getSource());
            });
    }
